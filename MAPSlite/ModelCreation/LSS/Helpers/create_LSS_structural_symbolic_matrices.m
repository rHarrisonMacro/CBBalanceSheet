...
if strcmpi(version('-release'),'2016b')
    ...
    mnemsInSymMatList = findsym(symMat);
    % Switch warning back on
    warning('on','symbolic:sym:DeprecateFindsym');
    ...
    %% UPDATED CODE TO DO THE EXTRACTION
    mnemsString = string(symvar(symMat));
    mnemsInSymMat = cellstr(mnemsString)';
end
end
