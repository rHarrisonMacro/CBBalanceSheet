function EASEprovRunData = ...
    convert_prov_run_dataset_from_MAPS_to_EASE_format(...
    Model,MAPSprovRunData)
% This helper converts provisional run data from MAPS to EASE format.
% It should be used as a helper to all EASE interfaces in MAPS that take as
% output a provisional run dataset from MAPS. It converts the structure
% provisional run dataset from MAPS to an equivalent cell array dataset for
% EASE.
%
% INPUTS:
%   -> Model: valid MAPS model
%   -> MAPSprovRunData: structure of forecast run data from MAPS
%
% OUTPUTS:
%   -> EASEprovRunData: cell array of forecast run data for EASE
%
% CALLS:
...
%
% DETAILS:
%   -> This helper translates the MAPS structure format for a provisional
%      run dataset to a cell array for use in EASE.
...
if modelIsLinearStateSpace
    [MAPSprovRunDataConfig,EASEprovRunDataConfig] = ...
        get_LSS_model_provisional_run_dataset_configs;
else
    [MAPSprovRunDataConfig,EASEprovRunDataConfig] = ...
        get_NLBL_model_provisional_run_dataset_configs;
...
EASEprovRunData = convert_dataset_from_MAPS_to_EASE_format(...
    Model,MAPSprovRunData,MAPSprovRunDataConfig,EASEprovRunDataConfig);
